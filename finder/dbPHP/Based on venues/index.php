...
if(isset($_GET['action'])&isset($_GET['date'])){
   ...
   // $date="$date 00:00:00";
    ...
    foreach($events as $event){
        ...
        */
            $sql="SELECT * FROM `genre` WHERE id = $venue[genre1]";
            $query=$pdo->query($sql);
            $venuegenre=$query->fetch(PDO::FETCH_ASSOC);
            $venuegenrename=$venuegenre['name'];
            
             $sql="SELECT * FROM `genre` WHERE id = $venue[genre2]";
            $query=$pdo->query($sql);
            $venuegenre=$query->fetch(PDO::FETCH_ASSOC);
            $venuegenrename2=$venuegenre['name'];
            ...
            if($venuegenrename2){
            $eventdetail['genres'][1]=$venuegenrename2;    
            }
        ...
        if($venuegenrename2){
          ...
          unset($event_artist_details['urls']);
 if($event['youtube']!=''){$event_artist_details['urls']['youtube']=$event['youtube'];}else{unset($event_artist_details['urls']['youtube']);}
        if($event['url']!=''){$event_artist_details['urls']['facebook']=$event['url'];}else{unset($event_artist_details['urls']['facebook']);}
